...
//#-end-hidden-code
//: First we'll do the [data flow](glossary://data%20flow) node. This one is pretty simple: it just takes two strings and concatenates them.
...
//: This is the trigger that will tell the program to assemble a `RepeatWhileNode` if it is connected.
    public var inputTrigger: InputTrigger? = InputTrigger()
    
//: Unlike the `TryCatchNode`, the `RepeatWhileNode` requires an input value to tell the loop if it needs to keep going. Even though this is a [control flow](glossary://control%flow) node, the input values act just like input values on [data flow](glossary://data%20flow) nodes. This won't complicate anything.
    public var inputValues: [InputValue] = [InputValue(id: "condition", name: "Condition", type: .bool)]
    
//: Similar to the `For` node, this ndoe has two outputs: one for the next node and one for the nodes that go inside the loop.
...
//: The Swift syntax for a repeat-while loop is like `repeat { /* loop */ } while /*condition */`.
//: The input value, out variable, and return statement are already written for you. Now you just need to use the `!+=` and `!!+=` operators to create the repeat-while loop.
    public func assemble() -> String {
