/*:
 The next major topic in visual programming is variables. Variables are always defiend by [output](glossary://output) [triggers](glossary://trigger) and are only accessible by [child nodes](glossary://child%20node). On the right, you can see that the `Declare Variable` node initializes a variable to `5` and then the `Set Variable` node sets the variable to `10`, which makes sense since the `Set Variable` node is a child of the `Declare Variable` node, so it can reference that variable.
 ...
 + Note:
 Later in this book, you'll have a chance to implement your own custom node with Swift. You can use the `Node.nearestControlNode` and `Node.availableVariables` variables which correspond to what was just discussed.
 ...
 Now try doubling the variable by setting the variable to two times itself. You'll need 4 nodes: `Set Variable`, `Get Variable`, `Multiply`, and `Integer Constant`. **Don't forget to make the `Set Variable` node the [nearest control node](glossary://nearest%20control%20node) before trying to reference the variable; otherwise, it will not work.**
